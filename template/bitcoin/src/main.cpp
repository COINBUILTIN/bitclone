...
const string strMessageMagic = "${bcl_name} Signed Message:\n";
...
static int64 nTargetTimespan = ${bcl_rate_secs}; 
static int64 nTargetSpacing = ${bcl_rate_secs}; 
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = ${bcl_init_val_block} * COIN;

    // Subsidy is cut in half every 4 years
    nSubsidy >>= (nHeight / ${bcl_halv_rate});
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	{
		nTargetTimespan = ${bcl_rate_secs};
		nTargetSpacing = ${bcl_rate_secs};
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "${bcl_name}", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
